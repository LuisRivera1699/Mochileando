...
const Travel = ({ titulo, descripcion, imagen, id, creador, points }: any) => {
    ...
    const fetchComments = async () => {
        ...
        const comentsResp = comments_aw.docs.map((doc) => ({
            id: doc.id,
            ...doc.data(),
        }));
    ...
    const performLike = async (e: any) => {
        ...
        await comment
            .update({
                usersWhoLike: firebase.firestore.FieldValue.arrayUnion(
                    usuario!!.uid
                ),
    ...
    const performDislike = async (e: any) => {
        ...
        await comment
            .update({
                usersWhoLike: firebase.firestore.FieldValue.arrayRemove(
                    usuario!!.uid
                ),
                likesCounter: increment,
            })
            ...
            .runTransaction((transaction) => {
                return transaction.get(comment).then((data) => {
                    if (!data.exists) {
                        ...
                        const usersWhoLike: Array<any> = data.data()!!
                            .usersWhoLike;
    ...
    const eliminarPublicacion = async () => {
        await app.firestore().collection("travells").doc(id).delete();
    ...
    return (
        <div className="travel-box">
            <svg
                onClick={liked ? performDislike : performLike}
                style={liked ? { fill: "blue" } : {}}
            ...
            {usuario?.uid === creadorid ? (
                        ...
                        if (edit) {
                            ...
                            if (localImg) {
                                let ref = app
                                    ...
                                    .child("publication")
                                    .child(usuario!!.uid);
                                const fileName = localImg.name;
                                ref = ref.child(fileName);
                                setNewImagen(ref.fullPath);
                                ref.put(localImg).then((snapshot) => {
                                    app.storage()
                                        .ref()
                                        .child(ref.fullPath)
                                        .getDownloadURL()
                ...
                >
                    <path
                        d="M28.125 6.5625L23.4375 1.875L4.6875 20.625L2.8125 27.1875L9.375 25.3125L28.125 6.5625ZM19.6875 5.625L24.375 10.3125L19.6875 5.625ZM4.6875 20.625L9.375 25.3125L4.6875 20.625Z"
                        stroke="black"
                        stroke-width="2"
            ...
            <div className="travel p-5 ">
                ...
                <div className="travel-image">
                    {edit ? (
                        <input
                            onChange={(event) => {
                                const imgNode = event?.target?.files || [];
                                const image = imgNode[0];
                                setLocalImg(image);
                            }}
                            type="file"
                            name="imagenes"
                        ...
                        <div className="mt-10">
                            ...
                            <div>
                                ...
                                {usuario?.uid === comment!.uid ? (
                                    <button
                                        onClick={() =>
                                            openEditComment(comment.id)
                                        }
                                        className="text-base focus:outline-none flex justify-center px-1 py-1 rounded cursor-pointer hover:bg-blue-700 hover:text-blue-100 bg-blue-100 text-blue-700 border duration-200 ease-in-out border-blue-600 transition"
                                    >
